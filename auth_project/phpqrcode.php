...
function generateLocalQRCode($data, $filename = '')
{
    if (!class_exists('QRcode') && file_exists('phpqrcode.php')) {
        include 'phpqrcode.php';
    }

    if (class_exists('QRcode')) {
        if (empty($filename)) {
            $filename = 'temp/qrcode_' . md5($data) . '.png';
        }
...
// Использование
$otpauth = "otpauth://totp/AuthSystem:$username?secret=$secret&issuer=AuthSystem";
$localQR = generateLocalQRCode($otpauth);

if ($localQR) {
    echo '<img src="' . $localQR . '" alt="QR Code" class="qr-code">';
} else {
    // Используем онлайн сервис как запасной вариант
    echo '<img src="https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=' . urlencode($otpauth) . '" alt="QR Code" class="qr-code">';
}

function generateBase64QRCode($data) {
    if (file_exists('phpqrcode.php')) {
        require_once 'phpqrcode.php';
...
// Использование
$otpauth = "otpauth://totp/AuthSystem:$username?secret=$secret&issuer=AuthSystem";
$base64QR = generateBase64QRCode($otpauth);

if ($base64QR) {
    echo '<img src="' . $base64QR . '" alt="QR Code" class="qr-code" style="width:300px;height:300px;">';
} else {
    // Запасной вариант
    echo '<div class="secret-display">' . chunk_split($secret, 4, ' ') . '</div>';
}
?>
